...
def insert_dp(dp_id: str, emb: list[float], cat=[]):
  print("Inserting data point id " + dp_id + " into vector search index " + str(
    search_index_id))
  try:
    if cat:
    ...
    print(
      res)  # If successful, the response body is empty [https://cloud.google.com/vertex-ai/docs/reference/rest/v1/projects.locations.indexes/upsertDatapoints].
...
def remove_dp(dp_id: str):
  print("Deleting data point id " + dp_id + " from vector search index" + str(
    search_index_id))
  try:
    ...
    print(
      res)  # If successful, the response body is empty[https://cloud.google.com/vertex-ai/docs/reference/rest/v1/projects.locations.indexes/removeDatapoints}
  except Exception as e:
    print("An error occurred:", e)
