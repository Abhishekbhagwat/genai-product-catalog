...
def join_categories(
        ...
        allow_trailing_nulls: bool = config.ALLOW_TRAILING_NULLS) -> dict[
                                                                     str:list[str]]:
            ...
            if row[col]:
                ...
                if allow_trailing_nulls:
                    if col == config.COLUMN_CATEGORIES[0]:
                        raise ValueError(
                            f'Top level category {col} for product {row[config.COLUMN_ID]} is null')
                    else:
                        break  # return existing categories
                else:
                    raise ValueError(
                        f'Column {col} for product {row[config.COLUMN_ID]} is null. To allow nulls update config.py')
    return categories
